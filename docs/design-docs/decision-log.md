...
## 2026-04-28
**决策主题**：免费内容增长与外部平台商业化优先  
**决定**：短期内网站内部不建设收费、会员或 Stripe 支付闭环；当前继续以免费内容、newsletter 订阅增长和影响力积累为主。若需要商业化验证，优先依赖外部平台承接付费内容、社群或服务转化。  
**理由**：当前主要瓶颈是受众规模、信任资产和内容节奏，而不是站内支付能力。外部平台可以更低成本验证付费意愿，避免在早期为未验证闭环增加复杂度。  
**影响**：`core-beliefs.md`、`ROADMAP.md`、`overall-requirements.md` 调整为 newsletter-first 与外部平台优先；站内会员、权限与 Stripe 仅保留为长期候选。  
**相关文件**：`docs/design-docs/core-beliefs.md`、`ROADMAP.md`、`docs/product-specs/overall-requirements.md`
